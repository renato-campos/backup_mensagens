...
def verificar_arquivos():
    ...
    try:
        ...
        messagebox.showerror(
            "Erro", "Os valores inicial e final devem ser números inteiros.")
        return
    ...
    for arquivo in arquivos:
        # Extrai os números no início do nome do arquivo
        numero_str = ''
        for char in arquivo:
            if char.isdigit():
                numero_str += char
    ...
    if numeros_fora_intervalo:
        relatorio.append(", ".join(map(str, sorted(numeros_fora_intervalo))))
    else:
        relatorio.append("Nenhum número encontrado fora do intervalo.")
    ...
    caminho_relatorio = os.path.join(
        pasta, f"relatorio_verificacao_{nome_pasta}.txt")
